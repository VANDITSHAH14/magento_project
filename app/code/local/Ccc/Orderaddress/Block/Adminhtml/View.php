<?php
class Ccc_Orderaddress_Block_Adminhtml_View extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        if ($this->checkAddress() == 1) {
            $confirmationMessage = $coreHelper->jsQuoteEscape(
                Mage::helper('sales')->__('Are you sure you want to Validate address?')
            );
            $this->_addButton('hii', array(
                'label' => Mage::helper('sales')->__('Validate'),
                'onclick' => 'setLocation(\'' . $this->addressValidation() . '\')',
            )
            );
        }
    ...
    {
        $order = $this->getOrder();
        return $order->getAddressValidationRequired();
    }
    ...
    {
        return $this->getUrl('*/sales_order/validateAddress');
    }
